    ...
    public void initializeEnvironment(String[] pArgs) throws Exception {
        try {
            ...
            if (!testBedArgumentPresent) {
                String lastSelectedTestbed = guiConfiguration.getString(StaticConfiguration.LAST_SELECTED_TESTBED_PROPERTY,
                      "default");
                String testbedConfigFileName = StaticConfiguration.TESTBED_CONFIG_DIRECTORY + "/" + lastSelectedTestbed + "."
                ...
                if (!new File(testbedConfigFileName).exists()) {
                    ...
                    if (testbedConfigFiles.length > 0) {
                        ...
                        guiConfiguration.setProperty(StaticConfiguration.LAST_SELECTED_TESTBED_PROPERTY,
                              testbedFileName.substring(0, testbedFileName.lastIndexOf('.')));
                    } else {
            ...
            props.setProperty("python.path",
                  StaticConfiguration.JYTHON_LIB + File.pathSeparator + StaticConfiguration.TEST_SCRIPT_DOC_TOOLS_DIR);
    ...
    private static void showUsage() {
        System.err.println(
              "Usage: <command> [-testsuite testsuiteDirectory] [-testbed configFileName.xml] [-engine engineFileName.xml] "
                    + "[-sutversion <sut_version_identifier>]");
        TestEngine.shutdown();
        System.exit(1);
    }
