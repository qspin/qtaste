...
public class ScriptCheckSyntaxValidator {
    ...
    private static final Pattern PEP_263_ENCODING_PATTERN = Pattern.compile("#.*coding[:=]\\s*([-\\w.]+)");
    private static final Pattern DO_STEP_PATTERN = Pattern.compile(
          "doStep\\(\\s*['\"]?([\\w.]+)['\"]?(?:\\s*,\\s*([\\w.]+))?\\s*\\)");
    //private static final Pattern DO_STEPS_PATTERN = Pattern.compile("doSteps\\(\\s*([\\w.]+)(?:\\s*,\\s*['\"]\\s*\\[(\\w*)
    // (?:\\s*-\\s*(\\w*))?\\]\\s*['\"]\\s*)?\\s*\\)");
    private static final Pattern STEPS_TABLE_DEF_PATTERN = Pattern.compile(
          "(\\w+)\\s*=\\s*[\\(\\[]\\s*(\\(\\s*['\"]?\\w+['\"]?\\s*,\\s*\\w+\\s*\\)(?:\\s*,\\s*\\(\\s*['\"]?\\w+['\"]?\\s*,"
                + "\\s*\\w+\\s*\\))*)\\s*[\\)\\]]");
    ...
    public boolean check() {
        ...
        // work-around for Jython ParserFacade.prepBufReader() bug which sets encoding to UTF-8 but fails when encoding is defined
        String scriptContent = PEP_263_ENCODING_PATTERN.matcher(mScriptContent).replaceFirst("");
