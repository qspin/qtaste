...
public class LineNumberPanel extends JComponent
       implements SyntaxComponent, PropertyChangeListener, DocumentListener,CaretListener  {
   ...
   protected void paintComponent(Graphics g) {
       ...
       for (int y = (clip.y / lh) * lh + lh + y_offset; y <= end; y += lh) {
           ...
           else
           {
               ...
               if (currentLine==lineNum) g.setFont(boldFont);
   ...
   public void setYOffset(int y_offset) {
    ...
    private void drawBreakpointHeader(Graphics g, int y, int line) {
        if (lastBreakEvent!=null)
        {
            if (lastBreakEvent.getAction() == TestScriptBreakpointEvent.Action.CONTINUE)
            {
                // do nothing 
            }
            if (lastBreakEvent.getAction() == TestScriptBreakpointEvent.Action.BREAK)
            {
