...
public class TestEngine {
    ...
    /**
     * Cancels start/stop of SUT. 
     * SUT stop in terminate() is not cancellable.
     */
    ...
    private static boolean startOrStopSUT(boolean start, TestResult tr) {
        ...
        if (hasControlScript()) {
            ...
            if (scriptFilename.endsWith(".py")) {
            	...
            	//if (javaHome!=null)
            		//scriptEngine = javaHome + "/bin/java -Dpython.path=" + jythonJar + File.pathSeparator + jythonLib + " -cp \"" + jythonHome + "/../build/jython-engine.jar"+ File.pathSeparator + jythonJar + File.pathSeparator + classPath + "\" org.python.util.jython";
            	//else
            	scriptEngine = "java -Dpython.path=" + jythonJar + File.pathSeparator + jythonLib + " -cp \"" + jythonHome + "/../build/jython-engine.jar"+ File.pathSeparator + jythonJar + File.pathSeparator + classPath + "\" org.python.util.jython";
            }
     ...
     * @return value of needToRestartSUT, which may be set to false if no control_script declared
     */
