...
public class OPCImpl implements OPC {
	...
	public OPCImpl(String instanceID) throws QTasteException
	{
		...
		opcUrl = TestBedConfiguration.getInstance().getMIString(m_instance,
															   "OPC",
															   "opc_url");
		opcPrefix = TestBedConfiguration.getInstance().getMIString(m_instance,
															   "OPC",
															   "opc_prefix");
		initialize();
	}
	...
	private void checkValueAsync(String varName, Object expectedValue, VariableType type, long timeout, Object precision) throws QTasteException {
				...
				if (currentValue != null &&
					...
					switch(type)
					{
						...
						case INT_TYPE: {
							...
							expectedValueNotReach = !((valueInt >= (expectedValueInt - precisionInt)) && (valueInt  <= (expectedValueInt + precisionInt)));
							break;
						}
						case DOUBLE_TYPE: {
							...
							expectedValueNotReach = !((valueDouble >= (expectedValueDouble - precisionDouble)) && (valueDouble  <= (expectedValueDouble + precisionDouble)));
							break;
						}
						case BOOL_TYPE: {
