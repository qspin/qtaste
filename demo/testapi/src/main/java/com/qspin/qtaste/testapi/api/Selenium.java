 ...
 */
public interface Selenium extends MultipleInstancesComponent {
    ...
    /** Launches the browser with a new  session */
    void start();

    /** Starts a new  testing session with a String, representing a configuration */
    public void start(String optionsString);

    /** Starts a new  testing session with a configuration options object */
    ...
    need to open an URL on another domain, use the TranslateApp Server to start a
    ...
    <p>This command can also be a useful workaround for bug SEL-339.  In some cases, TranslateApp will be unable to intercept a call to window.open (if the call occurs during or before the "onLoad" event, for example).
    In those cases, you can force TranslateApp to notice the open window's name by using the TranslateApp openWindow command, using
    ...
    Since "title" is not necessarily unique, this may have unexpected behavior.</p><p>If you're having trouble figuring out the name of a window that you want to manipulate, look at the TranslateApp log messages
    which identify the names of windows created via window.open (and therefore intercepted by TranslateApp).  You will see messages
    like the following for each window as it is opened:</p><p><code>debug: window.open call intercepted; window ID (which you can use with selectWindow()) is "myNewWindow"</code></p><p>In some cases, TranslateApp will be unable to intercept a call to window.open (if the call occurs during or before the "onLoad" event, for example).
    (This is bug SEL-339.)  In those cases, you can force TranslateApp to notice the open window's name by using the TranslateApp openWindow command, using
    an empty (blank) url, like this: openWindow("", "myFunnyWindow").</p>
    @param windowID the JavaScript window ID of the window to select
     */
    ...
    @param timeout a timeout in milliseconds, after which the action will return with an error.                If this value is not specified, the default TranslateApp                timeout will be used. See the setTimeout() command.
     */
    void waitForPopUp(String windowID, String timeout);

    /** <p>
    By default, TranslateApp's overridden window.confirm() function will
    return true, as if the user had manually clicked OK; after running
    this command, the next call to confirm() will return false, as if
    the user had clicked Cancel.  TranslateApp will then resume using the
    ...
    that TranslateApp's overridden window.confirm() function will normally automatically
    return true, as if the user had manually clicked OK, so you shouldn't
    need to use this command unless for some reason you need to change
    your mind prior to the next confirmation.  After any confirmation, TranslateApp will resume using the
    default behavior for future confirmations, automatically returning
    ...
    /** Instructs TranslateApp to return the specified answer string in response to
    ...
    alert is generated but you do not consume it with getAlert, the next TranslateApp action
    will fail.</p><p>Under TranslateApp, JavaScript alerts will NOT pop up a visible alert
    dialog.</p><p>TranslateApp does NOT support JavaScript alerts that are generated in a
    page's onload() event handler. In this case a visible dialog WILL be
    generated and TranslateApp will hang until someone manually clicks OK.</p>
    ...
    the next TranslateApp action will fail.
    </p><p>
    NOTE: under TranslateApp, JavaScript confirmations will NOT pop up a visible
    dialog.
    </p><p>
    NOTE: TranslateApp does NOT support JavaScript confirmations that are
    generated in a page's onload() event handler. In this case a visible
    dialog WILL be generated and TranslateApp will hang until you manually click
    OK.
    </p>
    ...
    do not get/verify it, the next TranslateApp action will fail.</p><p>NOTE: under TranslateApp, JavaScript prompts will NOT pop up a visible
    dialog.</p><p>NOTE: TranslateApp does NOT support JavaScript prompts that are generated in a
    page's onload() event handler. In this case a visible dialog WILL be
    generated and TranslateApp will hang until someone manually clicks OK.</p>
    ...
    object itself, so <code>this</code> will refer to the TranslateApp object.  Use <code>window</code> to
    ...
    /** Specifies whether TranslateApp should use the native in-browser implementation
    ...
    /** Specifies whether TranslateApp will ignore xpath attributes that have no
    ...
    /** Specifies the amount of time that TranslateApp will wait for actions to complete.
    ...
    (which are only available in the JS API).</p><p>TranslateApp constantly keeps track of new pages loading, and sets a "newPageLoaded"
    flag when it first notices a page load.  Running any other TranslateApp command after
    turns the flag to false.  Hence, if you want to wait for a page to load, you must
    wait immediately after a TranslateApp command that caused a page-load.</p>
    @param timeout a timeout in milliseconds, after which this command will return with an error
     */
    ...
    <p>TranslateApp constantly keeps track of new pages and frames loading,
    ...
    either show the log window in GUI mode, or enable browser-side logging in TranslateApp RC.
    ...
    TranslateApp's "getEval" command.  Beware that JS exceptions thrown in these script
    tags aren't managed by TranslateApp, so you should probably wrap your script
    ...
    /** Defines a new function for TranslateApp to locate elements on the page.
    For example,
    ...
    /** Loads script content into a new script tag in the TranslateApp document. This
    differs from the runScript command in that runScript adds the script tag
    to the document of the AUT, not the TranslateApp document. The following
    ...
    /** Removes a script tag from the TranslateApp document identified by the given
    ...
    @param fileLocator a URL pointing to the specified file. Before the file  can be set in the input field (fieldLocator), TranslateApp RC may need to transfer the file    to the local machine before attaching the file in a web page form. This is common in selenium  grid configurations where the RC server driving the browser is not the same  machine that started the test.   Supported Browsers: Firefox ("*chrome") only.
     */
    ...
    /** Kills the running TranslateApp Server and all browser sessions.  After you run this command, you will no longer be able to send
